...
export function SupervisorForm({ onSuccess }: SupervisorFormProps) {
  const { t } = useLanguage();
  const { register, handleSubmit, setValue, formState: { errors, isSubmitting } } = useForm<SupervisorFormData>();
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-[#0F172A]">Supervisor Registration</h2>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <button
          type="button"
          onClick={() => {
            setValue('email', `james.wilson.${Date.now()}@houseofvoice.io`);
            setValue('password', 'demo123');
            setValue('name', 'Dr. James Wilson');
            setValue('specialization', 'Clinical Supervision');
            setValue('years_experience', 15);
