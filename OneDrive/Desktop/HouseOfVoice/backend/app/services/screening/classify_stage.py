...
async def process_classification(
    ...
    """Combine stage outputs into final classification scores. Falls back gracefully."""
    try:
        speech_ratio = float(vad.get("speech_ratio", 0.82))
        speech_rate = float(features.get("speech_rate_wpm", 115.0))
        phoneme_scores: dict = phonemes.get("phoneme_scores", {})
        voice_stability = float(voice.get("voice_stability", 0.81))
        ...
        logger.warning(f"[classify_stage] Error: {e}. Using fallback.")
        return {
            "fluency_score": 0.72,
            "language_score": 0.65,
